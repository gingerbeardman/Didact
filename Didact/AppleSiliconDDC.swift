//  Copyright © @waydabber
//
//  Vendored from https://github.com/waydabber/AppleSiliconDDC (MIT).
//  Changes from upstream:
//   - the import line below: upstream pulls the private CoreDisplay/IOKit
//     symbols in via an Objective-C bridging header (AppleSiliconDDCObjC); here
//     they are declared in AppleSiliconDDCBridge.swift so the library drops into
//     an app target as a single Swift file.
//   - kIOMasterPortDefault → kIOMainPortDefault (deprecated since macOS 12).

// import AppleSiliconDDCObjC — replaced by AppleSiliconDDCBridge.swift (@_silgen_name shim)
import CoreGraphics
import Foundation
import IOKit
